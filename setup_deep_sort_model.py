...
from model import EmbeddingModel  # 作成したEmbeddingModelをインポート



# 作成した埋め込みモデルのロード
device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
embedding_model = EmbeddingModel(embedding_dim=128).to(device)
embedding_model.load_state_dict(torch.load("embedding_model.pth", map_location=device))
...
# 類似度の閾値
SIMILARITY_THRESHOLD = 0.85  # 類似度が閾値を超えた場合のみトラッキング
